...
**[Часть 2. Фильтрация и сортировка.](/ru/cobalt/developer/create-cobalt-field-base-functions/)**
...
1. `simpletext_base.xml` - `.xml` файл, содержит метаданные и параметры шаблонов поля.
...
Для выполнения базовых операций с полем (ввод, вывод, возможность полнотекстового поиска), класс поля должен содержать набор следующих предопределенных методов.
...
Вы можете вернуть или строку или массив данных. Нет необходимости его релиализовать. Это будет сделано автоматически. Вернете массив то и получите массив. Вернете строку то и получите строку в `$this->value`.
...
Т.е. по умолчанию шаблон вывода просто выводит переменную `$this->value;`.
...
Параметры поля в файлах шаблонов доступны через 
	...
	где parameter_name- имя параметра
